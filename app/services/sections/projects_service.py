...
from app.interfaces.database import ProjectSectionRepositoryProtocol
...
class ProjectSectionService:
    def __init__(self, db: ProjectSectionRepositoryProtocol) -> None:
        self._db = db
    ...
    async def create_project_section(
        ...
        count = await self._db.count_project_sections_for_profile(profile_id)
        ...
        result = await self._db.create_project_section(profile_id, data)
    ...
    async def list_project_sections(self, profile_id: int) -> list[ProjectSectionRead]:
        ...
        if not await self._db.profile_exists(profile_id):
            raise ValueError(f"Profile {profile_id} not found")
        result = await self._db.list_project_sections(profile_id)
        assert isinstance(result, list), "list_project_sections must return a list"
        return result

    async def get_project_section(self, profile_id: int, section_id: int) -> ProjectSectionRead:
        assert profile_id > 0, "profile_id must be a positive integer"
        assert section_id > 0, "section_id must be a positive integer"
        if not await self._db.profile_exists(profile_id):
            raise ValueError(f"Profile {profile_id} not found")
        result = await self._db.get_project_section(profile_id, section_id)
        if result is None:
    ...
    async def update_project_section(
        ...
        if not await self._db.profile_exists(profile_id):
            raise ValueError(f"Profile {profile_id} not found")
        result = await self._db.update_project_section(profile_id, section_id, data)
        if result is None:
            raise ValueError(f"Section {section_id} not found")
        return result

    async def delete_project_section(self, profile_id: int, section_id: int) -> None:
        assert profile_id > 0, "profile_id must be a positive integer"
        assert section_id > 0, "section_id must be a positive integer"
        if not await self._db.profile_exists(profile_id):
            raise ValueError(f"Profile {profile_id} not found")
        deleted = await self._db.delete_project_section(profile_id, section_id)
        if not deleted:
            raise ValueError(f"Section {section_id} not found")

    async def create_entry(
        self, profile_id: int, section_id: int, data: ProjectEntryCreate
    ) -> ProjectEntryRead:
        assert profile_id > 0, "profile_id must be a positive integer"
        assert section_id > 0, "section_id must be a positive integer"
        if not await self._db.profile_exists(profile_id):
        ...
        if section is None:
            raise ValueError(f"Section {section_id} not found")
        count = await self._db.count_project_entries(section_id)
        ...
        result = await self._db.create_project_entry(section_id, data)
        assert result.section_id == section_id, "returned entry section_id must match request"
        return result

    async def list_entries(self, profile_id: int, section_id: int) -> list[ProjectEntryRead]:
        assert profile_id > 0, "profile_id must be a positive integer"
        assert section_id > 0, "section_id must be a positive integer"
        if not await self._db.profile_exists(profile_id):
            raise ValueError(f"Profile {profile_id} not found")
        ...
        if section is None:
            raise ValueError(f"Section {section_id} not found")
        result = await self._db.list_project_entries(section_id)
        assert isinstance(result, list), "list_entries must return a list"
        return result

    async def update_entry(
        self, profile_id: int, section_id: int, entry_id: int, data: ProjectEntryUpdate
    ) -> ProjectEntryRead:
        assert profile_id > 0, "profile_id must be a positive integer"
        assert section_id > 0, "section_id must be a positive integer"
        ...
        if section is None:
            raise ValueError(f"Section {section_id} not found")
        result = await self._db.update_project_entry(section_id, entry_id, data)
        if result is None:
    ...
    async def delete_entry(self, profile_id: int, section_id: int, entry_id: int) -> None:
        ...
        if section is None:
            raise ValueError(f"Section {section_id} not found")
        deleted = await self._db.delete_project_entry(section_id, entry_id)
        if not deleted:
            raise ValueError(f"Entry {entry_id} not found")
